...
const VoiceFirstLanding: React.FC<VoiceFirstLandingProps> = ({
    ...
    React.useEffect(() => {
        ...
        if (SpeechRecognition) {
            ...
            recognition.continuous = true; // IMPORTANT: Continue listening until user stops
            ...
            recognition.onend = () => {
                ...
                // Don't auto-send - user will click send button
            };
            ...
            if (transcript.trim()) {
                ...
                setTimeout(() => {
                    setMessages(prev => [...prev, {
                        role: 'assistant',
                        text: "Parfait! J'ai bien noté. Quel est le montant de cette facture?"
                    }]);
                    setIsThinking(false);
                }, 1000);
            }
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 flex flex-col items-center p-8">
            ...
            `}
                    ...
                    {/* Live transcript */}
                    {transcript && (
                        <div className="mt-4 p-4 bg-indigo-500/20 rounded-xl border border-indigo-500/30 text-center">
                            <p className="text-indigo-200 italic">"{transcript}"</p>
                        </div>
                    )}
                </div>

                {/* Conversation - All on same page */}
